...
function validateManimCode(code: string): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
...
function createEnhancedPrompt(userMessage: string, errorContext?: string): string {
...
const generateManimScript = async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error("Error in generateManimScript:", err?.response?.data || err.message);
