...
const MANIM_SCRIPT_PATH = path_1.default.join(__dirname, "../../dist/manim/run.py");
const OUTPUT_DIR = path_1.default.join(__dirname, "../../dist/videos");
const OUTPUT_FILENAME = "output.mp4";
...
function parseManimeError(error) {
    // Extract meaningful error information from Manim error output
    const errorLines = error.split('\n');
    // Look for common error patterns
    const importantLines = errorLines.filter(line => line.includes('Error:') ||
...
function generateFallbackCode(originalPrompt) {
...
router.post("/", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        fs_1.default.mkdirSync(path_1.default.dirname(MANIM_SCRIPT_PATH), { recursive: true });
        ...
        while (attemptCount < maxRetries) {
            try {
                ...
                console.log(cleanedCode);
                // Write code to file
                fs_1.default.writeFileSync(MANIM_SCRIPT_PATH, cleanedCode);
                ...
                const command = `cd "${path_1.default.dirname(MANIM_SCRIPT_PATH)}" && manim -pql run.py ${sceneClass} -o ${OUTPUT_FILENAME}`;
                console.log("Executing command:", command);
                const { stdout, stderr } = yield execPromise(command, {
                    timeout: 60000, // 60 second timeout
                    cwd: path_1.default.dirname(MANIM_SCRIPT_PATH)
                });
                console.log("Manim stdout:", stdout);
                ...
                const expectedVideoPath = path_1.default.join(OUTPUT_DIR, OUTPUT_FILENAME);
                const mediaVideoPath = path_1.default.join(__dirname, `../../dist/manim/media/videos/run/480p15/${OUTPUT_FILENAME}`);
                let finalVideoPath = '';
                if (fs_1.default.existsSync(expectedVideoPath)) {
                    finalVideoPath = `/videos/${OUTPUT_FILENAME}`;
                }
                else if (fs_1.default.existsSync(mediaVideoPath)) {
                    finalVideoPath = `/manim/media/videos/run/480p15/${OUTPUT_FILENAME}`;
                }
                else {
                    // Look for any mp4 file in the output directory
                    const files = fs_1.default.readdirSync(path_1.default.join(__dirname, "../../dist/manim/media/videos/run/480p15/"));
                ...
                if (attemptCount < maxRetries) {
                    ...
                    try {
                        ...
                        const chatResponse = yield axios_1.default.post("http://localhost:3000/chat", {
        ...
        try {
            ...
            fs_1.default.writeFileSync(MANIM_SCRIPT_PATH, fallbackCode);
            const command = `cd "${path_1.default.dirname(MANIM_SCRIPT_PATH)}" && manim -pql run.py FallbackScene -o ${OUTPUT_FILENAME}`;
            yield execPromise(command, { timeout: 30000 });
            res.json({
                success: true,
